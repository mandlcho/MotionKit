...
class InstallThread(QThread):
    ...
    def run(self):
        ...
        try:
            motionkit_root = str(Path(__file__).parent)
            total = len(self.selected_dccs) + (1 if self.install_ue else 0)
            done = 0

            self.progress.emit(5, self.t['preparing'])

            for dcc in self.selected_dccs:
                name = dcc['name']
                path = Path(dcc['path'])

                if 'MotionBuilder' in name:
                    startup_dir = path / 'bin' / 'config' / 'PythonStartup'
                    startup_dir.mkdir(parents=True, exist_ok=True)
                    startup_file = startup_dir / 'motionkit_init.py'
                    content = (
                        "# MotionKit Startup Script\n"
                        "# Auto-generated by MotionKit installer\n"
                        "import sys\n"
                        "\n"
                        f"motionkit_root = r'{motionkit_root}'\n"
                        ...
                        "    print(f'[MotionKit ERROR] {e}')\n"
                        "    import traceback\n"
                        "    traceback.print_exc()\n"
                    )
                    startup_file.write_text(content, encoding='utf-8')

                elif '3ds Max' in name:
                    startup_dir = path / 'scripts' / 'Startup'
                    startup_dir.mkdir(parents=True, exist_ok=True)
                    startup_file = startup_dir / 'motionkit_init.ms'
                    template_file = Path(__file__).parent / 'motionkit_init.ms'
                    template = template_file.read_text(encoding='utf-8')
                    # MaxScript string literals use \\ for a literal backslash
                    ms_path = motionkit_root.replace('\\', '\\\\')
                    content = template.replace('PLACEHOLDER_PATH', ms_path)
                    startup_file.write_text(content, encoding='utf-8')

                done += 1
                pct = 10 + int(done / total * 80)
                self.progress.emit(pct, f"{self.t['configuring']} {name}")
            ...
            self.progress.emit(100, self.t['finalizing'])
